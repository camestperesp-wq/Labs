...
HORAS_SISTEMA = {f"{h:02d}:00-{h+2:02d}:00" for h in range(6, 22, 2)}
...
def normalizar_bloque_hora_sistema(hora_bloque):
    try:
        inicio, fin = hora_bloque.split("-")
        hora_inicio = int(inicio.split(":")[0])
        hora_fin = int(fin.split(":")[0])
    except Exception:
        return None
    ...
    hora_normalizada = f"{hora_inicio:02d}:00-{hora_fin:02d}:00"
    return hora_normalizada if hora_normalizada in HORAS_SISTEMA else None
...
def agrupar_por_pares(df):
    ...
    if col_monitor:
        ...
        print(" Columna 'MONITOR' no encontrada. Se usará cadena vacía.")
        ...
        while i < len(items):
            ...
            for j in range(i + 1, len(items)):
                ...
                if num_siguiente == num_actual + 1:
                    ...
                    hora_bloque = normalizar_bloque_hora_sistema(hora_bloque)
                    if not hora_bloque:
                        i = j + 1
                        encontrado_par = True
                        break
            ...
            if not encontrado_par:
                ...
                hora_sistema = normalizar_bloque_hora_sistema(hora_sistema)
                if not hora_sistema:
                    i += 1
                    continue
                resultado.append({
    ...
    if len(resultado) != len(resultado_limpio):
        print(f"    Eliminados {len(resultado) - len(resultado_limpio)} duplicados")
...
def importar_horario():
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        impares_eliminados = limpiar_bloques_impares_duplicados(conn)
        if impares_eliminados:
            print(f"\n Eliminados {impares_eliminados} bloques impares duplicados existentes")
        ...
        print(f"\n Eliminados {eliminados} registros de los laboratorios: {', '.join(labs_a_eliminar)}")
